...
void timer_ms_init_task(TimerMsTask* task, void (*isrs[TIMER_MS_MAX_TASK_SIZE])(void),
                        ...
                        uint8_t priority, uint8_t actionOnCompletion) {
...
/**
 * @brief Same as timer_ms_cancel except it returns a 1 if the task
 * was found and cancelled and a 0 if the task could not be found
 *
 * @param task The task to cancel
 * @return uint8_t 1 if the task was cancelled; else 0
 */
uint8_t timer_ms_cancel_task1(TimerMsTask* task) {
    ...
    // Task could not be found so nothing to cancel
    return 0;
}
